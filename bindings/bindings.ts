...
const url = new URL(
  "https://github.com/littledivy/webusb/releases/download/0.5.0",
  import.meta.url,
)
...
if (url.protocol !== "file:") {
  ...
  darwin = {
    aarch64: uri + "libwebusb_arm64.dylib",
    x86_64: uri + "libwebusb.dylib",
  }
}
...
const opts = {
  ...
  urls: {
    darwin,
    windows: uri + "webusb.dll",
    linux: uri + "libwebusb.so",
  },
  policy: undefined,
}
...
export function transfer_in(a0: Device, a1: number, a2: bigint) {
  const a0_buf = encode(JSON.stringify(a0))
